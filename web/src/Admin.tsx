...
export default function Admin() {
  ...
  const editSection = (patch: Partial<ManagedWorkSection>) => setSections((all) => all.map((section) => section.id === active ? { ...section, ...patch } : section))
  ...
  async function uploadAsset(file: File, kind: 'cover' | 'banner', id?: string) {
    if (!supabase) return
    ...
    if (kind === 'cover') editSection({ coverUrl: url }); else if (id) editItem(id, { banner_url: url })
  ...
  return <main className="admin">
    ...
    <button onClick={() => void saveAll()}>保存全部修改</button><p>{status}</p>
  </main>
}
