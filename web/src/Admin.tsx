...
export default function Admin() {
  ...
  async function login(e: FormEvent) { e.preventDefault(); if (!supabase) return; const { error } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: `${location.origin}/admin` } }); setStatus(error?.message || '登录链接已发送。') }
