...
import { loadWorks } from '../data/worksContent'
      ...
      <button className="wk-line-btn" onClick={() => onOpen(item)}>
        ...
        {hasMeta && (
          <span className="wk-line-meta">
            ...
            {tagsText && (
              <span className="wk-line-tag" title={tagsText}>
                {tagsText}
              </span>
            )}
          </span>
...
function SectionCard({
  ...
  const cover = section.coverUrl || SECTION_COVERS[section.id]
  return (
...
export default function Works({ lang, innerRef }: { lang: 'en' | 'zh'; innerRef: Ref<HTMLElement> }) {
  ...
  const [sections, setSections] = useState<WorkSection[]>(data.sections)
  useEffect(() => { loadWorks(lang).then(setSections) }, [lang])
